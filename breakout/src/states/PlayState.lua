...
function PlayState:update(dt)
	...
	if self.powerupSpawner >= 0 then
		self.powerupSpawner = self.powerupSpawner - dt
	else
		p = Powerup(math.random(0, VIRTUAL_WIDTH - 16), 0, (self.numLockedBricks > 0) and (not self.key))
		table.insert(self.powerups, p)
		self.powerupSpawner = 30
	end
	...
	for k, ball in pairs(self.balls) do
		...
		for k, brick in pairs(self.bricks) do
			...
			if brick.inPlay and ball:collides(brick) then
				...
				if brick.locked == true and self.key then
...
function PlayState:powerUp()
	for i = 0, 1 do
		...
		ball.x = self.paddle.x + self.paddle.width / 2 - 8
